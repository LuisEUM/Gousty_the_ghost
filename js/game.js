class Game {
    ...
    this.stages = [
      {
        ...
        map: [
          ...
          new Platform(
            this.ctx,
            0,
            this.ctx.canvas.height - 65,
            this.ctx.canvas.width,
            65,
            PLATFORMS_FOREST_FLOOR
          ),
        ],
        items: [],
      ...
      },
      {
        ...
        items: [new Item(this.ctx)],
        enemies: [
          new FastSlimes(this.ctx),
          new DarkSlimes(this.ctx, 0, -300),
          new DarkSlimes(this.ctx, 960, -300),
        ],
        background: new Background(this.ctx),
      },
      {
      ...
      },
      {
        ...
        items: [new Item(this.ctx)],
        enemies: [
          new DarkSlimes(this.ctx, 0, -300, true),
          new AngrySlimes(this.ctx),
          new DarkSlimes(this.ctx, 960, -300),
        ],
        background: new Background(this.ctx),
      },
      {
        ...
        items: [new Item(this.ctx)],
        enemies: [
          new SilverSlimes(this.ctx, 0, -600, true),
          new FastSlimes(this.ctx, 0, -100),
        ],
        background: new Background(this.ctx),
      },
      {
        ...
        items: [new Item(this.ctx)],
        enemies: [
          new SilverSlimes(this.ctx, 0, -300, true),
          new SilverSlimes(this.ctx, 960, -300),
          new FastSlimes(this.ctx, 0, -100),
        ],
        background: new Background(this.ctx),
      },
      {
        ...
        items: [new Item(this.ctx)],
        enemies: [
          new FireSlimes(this.ctx, 0, 400, true),
          new SwordSlimes(this.ctx, 900, 400, false),
        ],
        background: new Background(this.ctx),
      },
      {
        ...
        items: [],
        enemies: [
          new FastSlimes(this.ctx, 0, -300),
          new FastSlimes(this.ctx, 950, -300),
          new FireSlimes(this.ctx, 550, -300),
          new SilverSlimes(this.ctx, 0, -300),
          new SilverSlimes(this.ctx, 950, -300),
        ],
        background: new Background(this.ctx),
      },
      {
        ...
        items: [],
        enemies: [
          new FireSlimes(this.ctx, 0, null, true),
          new FireSlimes(this.ctx, 960, -300),
        ],
        background: new Background(this.ctx),
      },
      {
        ...
        enemies: [
          new FastSlimes(this.ctx, 0, -300),
          new FastSlimes(this.ctx, 200, -300),
          new FastSlimes(this.ctx, 400, -300),
          new FastSlimes(this.ctx, 600, -300),
          new FastSlimes(this.ctx, 1000, -300),
        ],
        background: new Background(this.ctx),
      },
      {
        ...
        enemies: [
          new SilverSlimes(this.ctx, 0, -300),
          new SilverSlimes(this.ctx, 950, -300),
          new SilverSlimes(this.ctx, 0, 290),
          new SilverSlimes(this.ctx, 950, 290),
        ],
        background: new Background(this.ctx),
      },
      {
        ...
        items: [],
        enemies: [
          new FireSlimes(this.ctx, 0, -300),
          new SilverSlimes(this.ctx, 950, -300),
          new FireSlimes(this.ctx, 950, -300),
          new SilverSlimes(this.ctx, 0, 290),
        ],
        background: new Background(this.ctx),
      },
      {
        ...
        enemies: [
          ...
          new SilverSlimes(this.ctx, 0, 290),
          new SilverSlimes(this.ctx, 950, 290),
          new SilverSlimes(this.ctx, 0, 400),
          new SilverSlimes(this.ctx, 950, 400),
        ],
        background: new Background(this.ctx),
      },
      {
        ...
        enemies: [
          ...
          new SilverSlimes(this.ctx, 0, 290),
          new SilverSlimes(this.ctx, 950, 290),
          new FireSlimes(this.ctx, 0, 400),
          new FireSlimes(this.ctx, 950, 400),
        ],
        background: new Background(this.ctx),
      },
    ];
  ...
  draw() {
    ...
    if (
      ...
      this.stages.length !== this.level + 1
    ) {
      this.arrwR.draw();
    }
  ...
  move() {
    ...
    if (this.transitions.length !== 0) {
      this.transitions.forEach((transition) =>
        transition.move(this.dark, this.transitions)
      );
    }
  }

  removeObjects() {
    this.enemies.forEach((monster, index) => {
  ...
  checkCollisions() {
    ...
    //Start monsterWave
    if (!this.stageCombat) {
      //se activa el colaider del centro
      const colMonX =
        400 <= this.player.x && //derecha del player
        600 >= this.player.x; //el mounstro esta a la izquierda
      const colMonY = 350 <= this.player.y; //abajo del player
      if (colMonX && colMonY) {
        //se activan los enemigos
        this.nextWave();
    ...
    //Next Stage
      ...
      const colNextY = 200 <= this.player.y; //abajo del player
      if (colNextX && colNextY) {
        //se activanla tansicion al siguiente mounstruo
        this.Ctransition();
        this.stageCombat = false;
        setTimeout(() => {
          this.nextLevel();
